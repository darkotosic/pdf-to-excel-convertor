...
from pdf_to_excel.models import (BoundingBox, DetectedGrid, DocumentWord,
                                 ReversDocument, WordSource)
...
def _word(text: str, x: float, y: float) -> DocumentWord:
    return DocumentWord(text, BoundingBox(x, y, x + max(8, len(text) * 5), y + 10),
                        1.0, 1, WordSource.NATIVE)
...
def test_blank_metadata_captions_are_not_values() -> None:
    ...
    words = [_word("(prezime i ime)", 30, 180),
...
def test_metadata_uses_relative_value_regions_and_preserves_identifier() -> None:
    ...
    words = [
        _word("Ђорђе", 30, 160), _word("Јовановић", 80, 160),
        _word("0112979710162", 310, 160),
        _word("Odeljenje", 430, 160), _word("IKT", 500, 160),
