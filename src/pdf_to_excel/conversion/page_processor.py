...
from pdf_to_excel.extraction import extract_generic_ruled_tables
...
class PageProcessor:
    ...
    def process(self, page: fitz.Page) -> PageResult:
        ...
        elif rendered is not None and raster_grids and words:
            result.tables = extract_generic_ruled_tables(raster_grids, words, page.number + 1)
        return result
...
def merge_document_words(
    native: list[DocumentWord], ocr: list[DocumentWord]
) -> tuple[list[DocumentWord], list[str]]:
